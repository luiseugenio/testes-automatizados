...
public class EntidadeNegocioComInterface {
	...
	private void validarRegras(Entidade entidade) throws Exception {
		...
		if (entidade.getDataInicial() != null
				&& entidade.getDataInicial().compareTo(new Date()) < 0) {
			throw new Exception(
					"A data inicial não pode ser menor que a data atual");
		}
		if (entidade.getDataInicial() != null
				&& entidade.getDataFinal().compareTo(entidade.getDataInicial()) < 0) {
			throw new Exception(
					"A data final não pode ser menor que a data inicial");
		}
		if (entidade.getTipoDocumento() != 1
				&& entidade.getTipoDocumento() != 2) {
			throw new Exception("Tipo de documento inválido");
		}
		if (entidade.getEmail() != null
				&& !(entidade.getEmail().contains("@") || entidade.getEmail()
						.contains("."))) {
	...
	public Entidade alterar(Entidade entidade) throws Exception {
		...
		Entidade entidadePersistida = persistencia.getById(entidade.getId());
